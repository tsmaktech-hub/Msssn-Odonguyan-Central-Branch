      ...
      <main className="max-w-7xl w-full mx-auto px-3 sm:px-6 lg:px-8 pt-4 sm:pt-6 space-y-4 sm:space-y-6">
        ...
        <div className="bg-white rounded-2xl sm:rounded-3xl p-3 sm:p-5 shadow-sm border border-slate-200 flex flex-col sm:flex-row items-center justify-between gap-3 sm:gap-4">
          ...
          <div className="flex items-center gap-2 sm:gap-3 w-full sm:w-auto justify-end">
            ...
            {attendees.length > 0 && onClearAllAttendees && (
              <button
                type="button"
                onClick={() => {
                  if (confirm('Are you sure you want to clear all names from the attendance sheet roster? This will remove all registered member names and their attendance check-ins.')) {
                    onClearAllAttendees();
                  }
                }}
                className="px-3 py-2 sm:px-4 sm:py-2.5 rounded-xl sm:rounded-2xl bg-red-50 hover:bg-red-100 text-red-700 font-semibold text-xs border border-red-200 flex items-center justify-center gap-1.5 transition-colors cursor-pointer"
                title="Clear all member names from the attendance sheet"
              >
                <Trash2 className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-red-600" />
                <span className="hidden sm:inline">Clear All Names</span>
              </button>
            )}
